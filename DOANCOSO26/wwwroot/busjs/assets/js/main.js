...
/* ===== BUSBUS FIX: nice select scroll runtime ===== */
(function ($) {
	function applyNiceSelectScrollFix() {
		$('.nice-select, .nice-select.open').css({
			'overflow': 'visible',
			'z-index': '100000'
		});
	...
	$(window).on('load', applyNiceSelectScrollFix);
	$(document).on('click', '.nice-select', function () {
		setTimeout(applyNiceSelectScrollFix, 10);
	});

	$(document).on('wheel', '.nice-select .list', function (event) {
		var originalEvent = event.originalEvent;
		var delta = originalEvent.deltaY;
		var atTop = this.scrollTop <= 0;
		var atBottom = Math.ceil(this.scrollTop + this.clientHeight) >= this.scrollHeight;
